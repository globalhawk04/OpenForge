...
def save_to_arsenal(new_item, tags, item_type="component"):
    ...
    # Duplicate Check (Robust Version)
    for existing in db['components']:
        # SAFETY FIX: Use .get() to prevent KeyError if bad data exists
        existing_name = existing.get('model_name', '').lower()
        existing_product = existing.get('product_name', '').lower() # Handle legacy keys
        
        # Check against both potential key names
        if existing_name == name or (existing_product and existing_product == name):
    ...
    entry = {
        ...
        "model_name": new_item['product_name'], # Standardize on model_name
        "price_est": new_item['price'],
        "specs": new_item['engineering_specs'],
        "visuals": new_item.get('visuals'),
        "image_url": new_item.get('reference_image'),
    ...
    with open(ARSENAL_FILE, "w") as f: json.dump(db, f, indent=2)
    print(f"      💾 Saved: {new_item['product_name']}")
